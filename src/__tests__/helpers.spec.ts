...
describe("Björklund's algorithm", () => {
  it('distributes 3 beats evenly into a grid of 5', () => {
    const result = bjorklund([[true], [true], [true], [false], [false]]);
    expect(result.flat(Infinity)).toEqual([true, false, true, false, true]);
  });

  it('distributes 2 beats evenly into a grid of 6', () => {
    const result = bjorklund([
      [true],
      [true],
      [false],
      [false],
    ...
    expect(result.flat(Infinity)).toEqual([
      true,
      false,
      false,
      true,
